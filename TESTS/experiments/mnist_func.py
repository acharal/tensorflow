...
def FuncImpl(x, W1, b1):
	hidden_out = tf.nn.relu(tf.add(tf.matmul(x, W1), b1))
	# y_est = tf.nn.softmax(tf.add(tf.matmul(hidden_out, W2), b2))
	return hidden_out

FuncImpl.add_to_graph(tf.get_default_graph())

x = tf.placeholder(tf.float32, [None, 784])
y = tf.placeholder(tf.float32, [None, 10])

y_est = func(x, W1, b1)
y_clipped = tf.clip_by_value(y_est, 1e-10, 0.9999999)
cross_entropy = tf.reduce_mean(tf.reduce_sum(y * tf.log(y_clipped) + (1 - y) * tf.log(1 - y_clipped), axis=1))
optimiser = tf.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(cross_entropy)
correct_prediction = tf.equal(tf.argmax(y, 1), tf.argmax(y_est, 1))
