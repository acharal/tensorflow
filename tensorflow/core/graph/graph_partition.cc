...
// An enter node for control flow.
Node* AddControlCall(Graph* g, const string& node_name,
                      const string& device_name, const string& frame_name,
                      const int call_id, Status* status) {
  NodeBuilder node_builder(node_name, "Call", g->op_registry());
  node_builder.Input({"dummy", 0, DT_FLOAT});
  node_builder.Attr("frame_name", frame_name);
  node_builder.Attr("call_id", call_id);
  Node* res_node;
  *status = node_builder.Finalize(g, &res_node);
  if (!status->ok()) return nullptr;
  res_node->set_assigned_device_name(device_name);
...
Node* AddControlMerge(const vector<string>& input_names, Graph g,
                      const string& node_name, const string& device_name,
                      Status* status) {
  NodeBuilder node_builder(node_name, "Merge", g->op_registry());
  int num_inputs = input_names.size();
  // check num_inputs > 1
  std::vector<NodeOut> inputs;
  inputs.resize(num_inputs);
  for (int i = 0; i < num_inputs; i++) {
    inputs[i] = {input_names[i] , i, DT_FLOAT};
  }
...
// A synthetic function, made up of dummy nodes. It performs control-flow actions
// on behalf of a leader on a different device.
// It consists of the different call nodes of this function.
struct ControlFunc {
...
Status AddControlFunc(const PartitionOptions& opts, Graph* g, const Node* src,
                      const Edge* edge, vector<Node*> func_calls,
                      std::vector<ControlFlowInfo>* cf_info,
                      ControlFunc* cf_func) {
  Status status;
  GraphDefBuilder::Options bopts(g, &status);
  ...
  if (func_calls.empty()) {
    return errors::InvalidArgument(
        "The control function ", curr_frame_name,
        " should have at least one call");
  }

  int num_calls = func_calls.size();
  std::unordered_set<int> call_ids;
  for (int i = 0; i < num_calls; i++) {
    Node* node = func_calls[i];
    int call_id;
    status = GetNodeAttr(node->attrs(), "call_id", &call_id);

    if (call_ids.find(call_id) != call_ids.end()) {
      break;
    }
    ...
    Node* call = AddControlCall(g, enter_name, device_name, frame_name,
                                  call_id, &status);
    if (!status.ok()) return status;
    cf_func.calls.enqueue(call);
    AddControlFlowInfo(call, src, cf_info);
    call_ids.insert(call_id);
  }

  if (cf_func.calls.size() == 1) {
    cf_func->entry = cf_func.calls[0];
  } else {
    const string& merge_name =
      ControlLoopName(opts.new_name(edge->dst()->name()));
    Node* merge = AddControlMerge(cf_func.calls, g, merge_name, device_name, &status);
    if (!status.ok()) return status;
...
Status AddControlFlow(const PartitionOptions& opts, Graph* g,
  ...
  // The map from frames (ie., function names) to a list of function calls.
  std::unordered_map<string, std::vector<Node*>> frame_call_map;
  num_node_ids = g -> num_node_ids();
  for (int i = 0; i < num_node_ids; ++i) {
    Node* node = g -> FindNodeId(i);
    if (node == nullptr) continue;
    ...
    if (IsCall(node)) {
      string frame_name;
      TF_RETURN_IF_ERROR(
          GetNodeAttr(node->attrs(), "frame_name", &frame_name));
      //DCHECK(!frame_name.empty());
      frame_call_map[frame_name].enqueue(node);
    }
  }
